...
public class JavaApplication1<Item> implements Iterable<Item> {
    ...
    public void recorrer(){
        for (Nodo x = nodoInicio.siguiente; x != null; x = x.siguiente) {
            System.out.println("En la lista: " + x.item);
        }
    }
    ...
    public void eliminar(Item item,JavaApplication1<String > primeraLista ){
        ...
        for(Nodo x = nodoInicio.siguiente; x != null; x = x.siguiente){
            ...
            if(x.siguiente!= nodoActual){
                
                if(x.item.equals(item)){
                    
                    Nodo nodoBorrar = x.siguiente;
                    nodoInicio.siguiente = nodoBorrar.siguiente;
                ...
                }else if(x.siguiente.item.equals(item)){
                    
                    Nodo nodoAntes = x;
                    Nodo nodoBorrar = x.siguiente;
                    
                    nodoAntes.siguiente= nodoBorrar.siguiente;
                    nodoBorrar.siguiente = null;
            ...
            }else if(x.siguiente ==(nodoActual)){
                    
                    if(x.siguiente.item.equals(item)){
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        */
        primeraLista.eliminar("118",primeraLista );
